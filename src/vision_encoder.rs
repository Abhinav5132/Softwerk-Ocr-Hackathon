...
pub fn apply_rotary_embedding(x: &Tensor, cos: &Tensor, sin: &Tensor) -> Result<Tensor> {
    ...
    let cos = Tensor::cat(&[cos, cos], candle_core::D::Minus1)?; //(dql_len, head_dim)
...
impl visionMlp {
    pub fn new(cfg: &VisionConfig, vb: VarBuilder) -> Result<Self> {
        ...
        let down_proj = candle_nn::linear_no_bias(cfg.hidden_size as usize, cfg.intermediate_size as usize, vb.pp("down_proj"))?;
...
impl visionEncoder {
    ...
    pub fn forward(&self, pixel_values: &Tensor) -> Result<Tensor> {
        ...
        let (cos, sin) = self.rope.freqs(h, w, device, d_type)?;
